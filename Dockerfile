...
ARG NimCommand="c"
ARG NimMain="src/minigrid.nim"
RUN nim $NimCommand \
  $NimFlags \
  --nimcache:/tmp/minigrid-nimcache \
  --out:minigrid \
  $NimMain && \
  nim c \
  $NimFlags \
  --nimcache:/tmp/minigrid-player-nimcache \
  --out:minigrid-player \
  src/minigrid_player.nim
...
WORKDIR /workspace/minigrid
COPY --from=build /workspace/minigrid/minigrid /bin/minigrid
COPY --from=build /workspace/minigrid/minigrid-player /bin/minigrid-player
COPY --from=build /workspace/minigrid/*.json ./
COPY --from=build /workspace/minigrid/data ./data
