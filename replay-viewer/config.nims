...
# use-after-free fixed in minigrid_replay.nim (emscripten_exit_with_live_runtime)
...
# minigrid_error_len(). Aborting keeps linear memory intact, and the page reads
# minigrid_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "minigrid_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_minigrid_load_replay,_minigrid_frame,_minigrid_input,_minigrid_packet_ptr,_minigrid_packet_len,_minigrid_mismatch_tick,_minigrid_error_ptr,_minigrid_error_len,_minigrid_stage_ptr,_minigrid_stage_len
  """).replace("\n", " ")
)
